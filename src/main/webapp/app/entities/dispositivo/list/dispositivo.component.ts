...
import { DispositivoService, EntityArrayResponseType } from '../service/dispositivo.service';
...
export class DispositivoComponent implements OnInit {
  ...
  trackId = (item: IDispositivo): number => this.dispositivoService.getDispositivoIdentifier(item);
  ...
  protected fillComponentAttributesFromResponseBody(data: IDispositivo[] | null): IDispositivo[] {
    ...
    if (this.links().prev) {
      ...
      if (data) {
        for (const d of data) {
          if (dispositivosNew.some(op => op.id === d.id)) {
