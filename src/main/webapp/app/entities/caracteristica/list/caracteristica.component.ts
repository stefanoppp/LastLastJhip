...
import { CaracteristicaService, EntityArrayResponseType } from '../service/caracteristica.service';
...
export class CaracteristicaComponent implements OnInit {
  ...
  trackId = (item: ICaracteristica): number => this.caracteristicaService.getCaracteristicaIdentifier(item);
  ...
  protected fillComponentAttributesFromResponseBody(data: ICaracteristica[] | null): ICaracteristica[] {
    ...
    if (this.links().prev) {
      ...
      if (data) {
        for (const d of data) {
          if (caracteristicasNew.some(op => op.id === d.id)) {
